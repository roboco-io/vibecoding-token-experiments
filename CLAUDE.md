...
5. **대시보드 아티팩트 갱신 + README 링크 동기화**: 대시보드 아티팩트(랄프 루프 모델별 완주 비교, `137de971-...`)를 새 실험 데이터로 갱신할 때는 반드시 기존 URL로 재배포한다(Artifact 도구에 `url` 전달 — 새 URL 발급 금지). README `## 실험 결과` 절 상단의 "라이브 대시보드" 링크가 그 URL을 가리키는지, "최신 실험 반영: EXP-NNN (날짜)" 표기를 이번 실험으로 갱신했는지 확인한다. 부득이 새 아티팩트를 만든 경우 README 링크를 새 URL로 교체한다.
