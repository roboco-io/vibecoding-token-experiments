#!/bin/bash
# EXP-014: Claude Code x kimi-k3 직결(ANTHROPIC_BASE_URL) 랄프 루프 (EXP-013 driver에서 모델 env만 치환, n=1)
# 기동: nohup caffeinate -is "$HOME/ralph-exp017k/driver.sh" > /dev/null 2>&1 &
source "$HOME/.zsh_secrets"
BASE="$HOME/ralph-exp017k"
RUN="kimi-1"
MAX_ITER=30

REPO="$BASE/app-$RUN"; MET="$BASE/metrics-$RUN.csv"; LOG="$BASE/ralph-run-$RUN.log"
[ -f "$BASE/done-$RUN" ] && exit 0
[ -n "$KIMI_API_KEY" ] || { echo "KIMI_API_KEY missing" >> "$BASE/driver.log"; exit 1; }
mkdir -p "$REPO"
...
echo "=== RUN $RUN (direct kimi-k3) start: $(date '+%F %T') ===" >> "$BASE/driver.log"
