#!/bin/bash
# EXP-015: Claude Code x solar-open2 직결(ANTHROPIC_BASE_URL) 랄프 루프 (EXP-014 driver에서 모델 env만 치환, n=1)
# 기동: nohup caffeinate -is "$HOME/ralph-exp015/driver.sh" > /dev/null 2>&1 &
...
echo "=== RUN $RUN (direct solar-open2) start: $(date '+%F %T') ===" >> "$BASE/driver.log"
