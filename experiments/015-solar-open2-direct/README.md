...
EXP-013(qwen3.8-max)·EXP-014(kimi-k3)에서 직결 스택이 제공자 독립적으로 재사용됨을 확인했다. 본 실험은 같은 템플릿을 **Upstage solar-open2**에 적용한다. (정정 2026-08-04: 설계 초판은 EXP-008을 ccr 스택으로 오기했으나, EXP-008은 이미 Upstage 공식 직결이었다 — ralph.sh 실측.) 따라서 본 실험의 의의는 스택 교체가 아니라 **EXP-008 직결 완주(iter 10, 약 173분)를 정본 프롬프트(EXP-010, byte-identical)·EXP-013/014 표준 하네스 조건에서 재현**하는 것이며, 직결 템플릿의 3사(DashScope·Moonshot·Upstage) 이식성 확인을 겸한다. EXP-008과는 프롬프트·시점·상류 서빙이 달라 정량 비교가 아닌 정성 참조로 한정한다(사전 등록).
...
- 완주 시: EXP-008(직결·정본 이전 프롬프트)과 동일 모델·동일 스택 정성 비교 — 프롬프트 정본화의 효과 관찰 (시점 교락 명시 전제)
