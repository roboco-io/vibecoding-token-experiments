#!/bin/bash
# EXP-020: Claude Code x solar-pro4 직결(ANTHROPIC_BASE_URL) 랄프 루프 — EXP-015 driver 이식 (모델 ID만 치환, RUN 인자화)
RUN="$1"
...
[ -n "$UPSTAGE_API_KEY" ] || { echo "UPSTAGE_API_KEY missing" >> "$BASE/driver.log"; exit 1; }
mkdir -p "$REPO"
...
echo "=== RUN $RUN (direct solar-pro4) start: $(date '+%F %T') ===" >> "$BASE/driver.log"
for i in $(seq $((DONE_ITER + 1)) $MAX_ITER); do
  ...
  env -u ANTHROPIC_API_KEY \
    ...
    ANTHROPIC_MODEL=solar-pro4 \
    ANTHROPIC_SMALL_FAST_MODEL=solar-pro4 \
    ANTHROPIC_DEFAULT_HAIKU_MODEL=solar-pro4 \
